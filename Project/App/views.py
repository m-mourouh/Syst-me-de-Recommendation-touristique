        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                if form.cleaned_data['password'] != form.cleaned_data['confirmpass']:
                    data = {'errors': {'confirmpass':'','err':400}}
                    data['errors']['confirmpass'] = 'les deux mots de passe ne sont pas identiques'
                    return JsonResponse(data)
                else:
                ...
                if request.POST['password'] != request.POST['confirmpass']:
                    data['errors']['confirmpass'] = 'les deux mots de passe ne sont pas identique'
                return JsonResponse(data)
        else:
            form = ClientForm()
